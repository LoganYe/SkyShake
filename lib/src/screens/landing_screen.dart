...
class _LandingScreenState extends State<LandingScreen> {
  ...
  Future<void> _handleAnalyze() async {
    ...
    try {
      final repository = context.read<TrackingRepository>();
      final analysis = await repository.analyzeRoute(
        RouteQuery(
  ...
  Future<void> _openAppShell() async {
    await Navigator.of(
      context,
                    ...
                    child: ConstrainedBox(
                      ...
                      child: SingleChildScrollView(
                        ...
                        child: Column(
                          ...
                          children: [
                            _TopBar(onOpenApp: _openAppShell),
                            const SizedBox(height: 20),
                            _HeroPanel(
                              compact: compactHero,
                              onPreview: _scrollToPreview,
                              onOpenApp: _openAppShell,
                            ),
...
class _TopBar extends StatelessWidget {
  const _TopBar({required this.onOpenApp});
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        ...
        const Spacer(),
        OutlinedButton.icon(
          key: const Key('landing-open-app-button'),
          onPressed: onOpenApp,
    ...
    final copy = Column(
      ...
      children: [
        ...
        Wrap(
          ...
          children: [
            FilledButton.icon(
              key: const Key('landing-run-route-button'),
              onPressed: onPreview,
            ...
            OutlinedButton.icon(
              ...
              label: const Text('Open Flight Lookup'),
            ),
          ],
        ),
        const SizedBox(height: 14),
        Text(
          'The preview on this page is live. Flight-number lookup lives inside the app shell.',
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        Text(
          'This page stays short on purpose. Explain the job, let people try it, and be explicit about where the model stops.',
          style: theme.textTheme.bodyLarge?.copyWith(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const _BoundaryRow(
              title: 'Weather-backed, not cabin truth',
              body:
                  'SkyShake models route turbulence from weather data. It does not guarantee what a specific cabin will feel like.',
            ),
            const SizedBox(height: 12),
            const _BoundaryRow(
              title: 'Flight lookup is provider-dependent',
              body:
                  'The app shell can look up flight numbers, but that path is only live when a provider key is configured for the environment.',
            ),
          ],
        ),
      ),
